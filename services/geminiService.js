...
import { IMAGE_GENERATION_PROMPT, VIDEO_GENERATION_PROMPT, API_KEY } from '../constants.js';
 ...
 * @returns A Promise that resolves to a base66 encoded string of the generated PNG image.
 * @throws Will throw an error if the image generation fails or the API does not return an image.
 */
export const generateFamilyPhoto = async (files) => {
 ...
 */
export async function* generateFamilyVideo(base64Image, imageMimeType, numFamilyMembers) {
    const ai = new GoogleGenAI({ apiKey: API_KEY });
